...
<div class="home">
  ...
  <div class="wrap" ref="loadmore">
    ...
    <Table border @row-click="rowClick" size="mini" ref="table_data" style="width: 100%;position:fixed;z-index:50;" :height="36">
      <table-column :label="$t('navbar.取款金额')" width></table-column>
      <table-column :label="$t('POS.Balance')" width></table-column>
      <table-column :label="$t('POS.operator')" width="150"></table-column>
    </Table>
    <mu-load-more @refresh="refresh" :refreshing="refreshing" :loading="loading" @load="load" :loaded-all="loadedAll">
      <Table :data="list" border @row-click="rowClick" size="mini" ref="table_data1" style="width: 100%;">
        <table-column prop="PayMoney" width></table-column>
        <table-column align prop="Money" width></table-column>
  ...
  <mu-dialog :title="$t('Common.xiangqing')" width="360" scrollable :open.sync="openSimple" class="box_info">
    <mu-list class="box">
      ...
      <mu-list-item button :ripple="false">
        ...
        <mu-list-item-title>{{rowdata.Money}}</mu-list-item-title>
      </mu-list-item>
      <mu-divider></mu-divider>
      <mu-list-item button :ripple="false">
        <mu-list-item-action>
...
import {
  WithdrawalPOSUserChangeCardRecord
} from "@/api/getdata";
import {
  parseTime
} from "@/utils/index";
