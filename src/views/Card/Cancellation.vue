...
<div class="home">
  ...
  <div class="wrap" ref="loadmore">
    ...
    <Table border @row-click="rowClick" size="mini" ref="table_data" style="width: 100%;position:fixed;z-index:50;" :height="36">
      <table-column :label="$t('POS.operator')" width="90"></table-column>
      <table-column :label="$t('TableFields.CardExchangeSchedule.NewPOSCard')" width></table-column>
      <table-column :label="$t('TableFields.CardExchangeSchedule.ExchangeDate')" width="150"></table-column>
    </Table>
    <mu-load-more @refresh="refresh" :refreshing="refreshing" :loading="loading" @load="load" :loaded-all='loadedAll'>
      <Table :data="list" border @row-click="rowClick" size="mini" ref="table_data1" style="width: 100%;">
        <table-column prop="UserName" width="90"></table-column>
        <table-column align prop="POSCard" width></table-column>
        <table-column width="150">
          <template slot-scope="scope">{{scope.row.LogDate}}</template>
        </table-column>
      </Table>
    </mu-load-more>
  </div>
  <mu-drawer style="width:7.4667rem" :open.sync="open" :docked="false" right="right">
    ...
    <mu-row justify-content="center" class="mu-person-form">
      <mu-form ref="form" :model="searchitem" label-width="80">
        <mu-form-item label-position="right" :label="$t('POS.operator')+':'">
  ...
  <mu-dialog :title="$t('Common.xiangqing')" width="360" scrollable :open.sync="openSimple" class="box_info">
    <mu-list class="box">
      <mu-list-item button :ripple="false">
        ...
        <mu-list-item-title>{{rowdata.UserName}}</mu-list-item-title>
      </mu-list-item>
      <mu-divider></mu-divider>
      <mu-list-item button :ripple="false">
        <mu-list-item-action>
          <!-- <mu-switch  readonly></mu-switch> -->
          {{$t('TableFields.CardExchangeSchedule.NewPOSCard')}}：
        </mu-list-item-action>
        <mu-list-item-title>{{rowdata.NewPOSCard}}</mu-list-item-title>
      </mu-list-item>
      <mu-divider></mu-divider>
      <mu-list-item button :ripple="false">
        <mu-list-item-action>
          <!-- <mu-switch  readonly></mu-switch> -->
          {{$t('TableFields.CardExchangeSchedule.ExchangeDate')}}：
        </mu-list-item-action>
        <mu-list-item-title>{{rowdata.ExchangeDate}}</mu-list-item-title>
      </mu-list-item>
    </mu-list>
