      ...
      <Table :data="list" border @row-click="rowClick" size="mini" ref="table_data1" style="width: 100%;">
        <table-column type="expand">
          <template slot-scope="props">
            ...
            {{$t('Att.Leave_reason')}}：<span>{{props.row.rDesc}}</span>
            <br />
            <br />

            {{$t('navbar.请假天数')}}：<span>{{props.row.rDay}}</span>

          </template>
        </table-column>
        <table-column prop="BeginDate" :label="$t('Meet.Starting_time')" width='140'></table-column>
        <table-column align prop="EndDate" :label="$t('Meet.End_Time')"  width='140'></table-column>
  ...
  <mu-dialog :title="$t('Common.xiangqing')" width="360" scrollable :open.sync="openSimple" class="box_info">
    <mu-list class="box">
      ...
      <mu-list-item button :ripple="false">
        ...
        <mu-list-item-title>{{rowdata.rDay}}</mu-list-item-title>
      </mu-list-item>
    </mu-list>
