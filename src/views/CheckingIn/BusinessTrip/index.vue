      ...
      <Table :data="list" border @row-click="rowClick" size="mini" ref="table_data1" style="width: 100%;">
        <table-column type="expand">
          <template slot-scope="props">
            ...
            {{$t('Att.Reason_for_business_trip')}}：<span>{{props.row.rDesc}}</span>
            <br />
            <br />

            {{$t('Att.Travel_legnth')}}：<span>{{props.row.rDay}}</span>

          </template>
        </table-column>
        <table-column prop="BeginDate" label=" " width='140'></table-column>
        <table-column align prop="EndDate" label=" " width="140"></table-column>
        <table-column label=" ">
          <template slot-scope="scope">{{scope.row.rMinute}}</template>
        </table-column>
      </Table>
    </mu-load-more>
  </div>
...
import {
  WorkReportGetWorkEgressionRecordList
} from "@/api/CheckingIn";
import {
  parseTime
} from "@/utils/index";
