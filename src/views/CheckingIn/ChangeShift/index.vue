      ...
      <Table :data="list" border @row-click="rowClick" size="mini" ref="table_data1" style="width: 100%;">
        <table-column type="expand">
          <template slot-scope="props">
            ...
            {{$t('Att.Original_working_hours')}}:<span>{{props.row.wn1}}</span>
            <br />
            <br />

            {{$t('Att.New_working_hours')}}：<span>{{props.row.wn2}}</span>

          </template>
        </table-column>
        <table-column prop="rDate1" width="140"></table-column>
        <table-column align prop="rDate2" width=""></table-column>
        <!-- <table-column > -->
        <!-- <template slot-scope="scope">{{scope.row.PortNum==1?'入门':'出门'}}</template> -->
        <!-- </table-column> -->
      </Table>
    </mu-load-more>
  </div>
    ...
    <mu-list class="box">
      <mu-list-item button :ripple="false">
        <mu-list-item-action>
          ...
          {{$t('Att.Original_working_hours')}}：
        </mu-list-item-action>
        <mu-list-item-title>{{rowdata.wn1}}</mu-list-item-title>
      </mu-list-item>
      <mu-divider></mu-divider>
      <mu-list-item button :ripple="false">
        <mu-list-item-action>
          <!-- <mu-switch  readonly></mu-switch> -->
          {{$t('Att.New_working_hours')}}：
        </mu-list-item-action>
        <mu-list-item-title>{{rowdata.wn2}}</mu-list-item-title>
      </mu-list-item>
    </mu-list>
...
import {
  WorkReportGetWorkEditRecordList
} from "@/api/CheckingIn";
import {
  parseTime
} from "@/utils/index";
