...
<div class="home">
  ...
  <div class="wrap" ref="loadmore">
    ...
    <Table border @row-click="rowClick" size="mini" ref="table_data" style="width: 100%;position:fixed;z-index:50;" :height="36">
      ...
      <table-column :label="$t('navbar.打卡日期')" width="150"></table-column>
      <table-column :label="$t('navbar.打卡模式')"></table-column>
    </Table>
    <mu-load-more @refresh="refresh" :refreshing="refreshing" :loading="loading" @load="load" :loaded-all="loadedAll">
      <Table :data="list" border @row-click="rowClick" size="mini" ref="table_data1" style="width: 100%;">
        ...
        <table-column align prop="DataTime" :label="$t('navbar.打卡日期')" width="150"></table-column>
        <table-column :label="$t('Att.status')">
          <template slot-scope="scope">{{scope.row.RecordType==1?$t('navbar.设备打卡'):$t('Att.AttendSign')}}</template>
        </table-column>
      </Table>
    </mu-load-more>
  </div>
