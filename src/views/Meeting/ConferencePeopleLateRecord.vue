  ...
  <div class="home">
    ...
    <div class="wrap" ref="loadmore">
      ...
      <Table
        ...
        :height="36"
      >
        <table-column label="门名称" width='150'></table-column>
        <table-column label="日期" width='140'></table-column>
        <table-column label="状态" ></table-column>
      </Table>
      <mu-load-more @refresh="refresh" :refreshing="refreshing" :loading="loading" @load="load" :loaded-all='loadedAll'>
        <Table
          ...
          style="width: 100%;"
        >
          <table-column prop="DoorName" label="门名称" width="150"></table-column>
          <table-column align prop="RecordTime" label="日期" width='140'></table-column>
          <table-column label="状态" >
            <template slot-scope="scope">{{scope.row.PortNum==1?'进门':'出门'}}</template>
          </table-column>
        </Table>
      </mu-load-more>
    </div>
...
import { ConferencePeopleLateRecord } from "@/api/getdata";
...
export default {
  ...
  methods: {
    ...
    getdata() {
      let that = this;
      ConferencePeopleLateRecord({ PageIndex: this.pageindex, PageSize: this.pagesize })
        .then(res => {
    ...
    searchdata(){
      ...
      ConferencePeopleLateRecord({ PageIndex: this.pageindex, PageSize: this.pagesize,...param})
        .then(res => {
          // console.log(res.RetData);
          that.list=[]
          that.list = [...that.list, ...res.RetData.Data];
